...
@router.get("/best/{player_id}", dependencies=[Depends(verify_auth0_token)])
async def get_best(
    player_id: int, service: BiddingService = Depends(get_bidding_service)
):
    # The service handles the repo logic
    return service.offer_repo.get_best_offers_for_player(player_id)


@router.post("/save", dependencies=[Depends(verify_auth0_token)])
async def save_offer(
    offer: Offer, service: BiddingService = Depends(get_bidding_service)
):
...
@router.post("/timeout", dependencies=[Depends(verify_auth0_token)])
async def auction_timeout(
    ...
    await admin_router.manager.send_personal_message(
        json.dumps({"auctionClosed": auction_result["auctionClosed"]}), ADMIN_USER_ID
    )
    return service.resolve_auction(payload.user_id, payload.player_id)


@router.post("/second_round/save")
async def save_second_round_offer(
    offer: Offer,
    user_payload: dict = Depends(verify_auth0_token),
    service: BiddingService = Depends(get_bidding_service),
):
    # Logic and Validation are now inside the Service
    # If validation fails, the Service raises HTTPException, which FastAPI handles automatically
    ...
    if offer_saved:
        await admin_router.manager.broadcast(
            json.dumps({"bestOffer": offer.model_dump(mode="json")})
        )
    return offer_saved
